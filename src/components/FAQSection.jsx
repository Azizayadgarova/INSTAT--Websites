...
const faqData = [
	...
	},
	{
		...
		answer: 'We offer flexible plans for individuals, teams, and enterprises.',
	},
]
	...
	const renderCard = (item, realIndex, delay = 0) => {
		...
		return (
			<AnimatedSection key={realIndex} delay={delay}>
				<div
					className={`${
						platformStyle ? '' : 'faq-card'
					} relative overflow-hidden ${
						isOpen ? 'shadow-[0_10px_30px_rgba(0,0,0,0.4)]' : ''
					}`}
					style={
						platformStyle
							? {
				...
				>
					...
					>
						<span
							className={`text-[15px] sm:text-[17px] md:text-[20px] font-medium transition-colors ${
						...
						<div
							className={`transition-transform duration-300 flex-shrink-0 ${
								isOpen ? 'rotate-45 text-[#4FC3F7]' : 'text-white'
							}`}
						>
							<svg
								className='w-8 h-8'
