...
const FloatingCard = memo(({ card, index, progress, totalCards = 5 }) => {
	...
	const stay = !!card.stayOnExit

	const z = useTransform(progress, [0, mid, end], [index * -1200, 50, stay ? 50 : 1200])
	const scale = useTransform(progress, [start, mid, end], [1 - index * 0.18, 1.15, stay ? 1.15 : 1.5])
	...
	const y = useTransform(
		progress,
		[start, mid, end],
		[`${initialYPos}%`, '0%', stay ? '0%' : '-250%'],
	)
	const x = useTransform(
		progress,
		[start, mid, end],
		[`${card.startX}`, '0%', stay ? '0%' : `${card.exitX}`],
	)

	const opacity = useTransform(progress, [start, mid, end - 0.1, end], [0.6, 1, 1, stay ? 1 : 0])
	...
	return (
		...
		>
			<div className={`flex items-center gap-6${card.plainText && index % 2 !== 0 ? ' flex-row-reverse' : ''}`}>
				<div className='relative w-64 h-44 md:w-[720px] md:h-[350px] rounded-3xl overflow-hidden '>
					<img
						src={card.img}
						alt={card.title}
						width={720}
